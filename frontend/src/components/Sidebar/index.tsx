...
import { signin } from '../../Reducers/Admin.js';
...
console.log(admin);
  ...
  useEffect(
    () => {
      ...
      if (lsAdmin) {
        dispatch(signin({ admin: JSON.parse(lsAdmin), token: lstoken }));
      }
    }, []
  )
  useEffect(
  ...
  return (
   ...
   {/* <!-- Menu Item Employes --> */}
   {admin.data && admin.data.role !== 'employee' && ( 
   <li>
                <NavLink
                  to="/employes"
                  className={`group relative flex items-center gap-2.5 rounded-sm py-2 px-4 font-medium text-bodydark1 duration-300 ease-in-out hover:bg-graydark dark:hover:bg-meta-4 ${
                    pathname.includes('employes') &&
                    'bg-graydark dark:bg-meta-4'
                  }`}
                >
                  <svg
