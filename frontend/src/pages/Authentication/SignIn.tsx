...
interface SuccessResponse {
  status: boolean;
  admin: any; // Replace 'any' with the actual type for admin
  token: string;
  msg: string;
}
...
const SignIn: React.FC = () => {
  ...
  const loginHandler = (e: React.FormEvent<HTMLFormElement>) => {
    ...
    if (data.email !== "" && data.password !== "" && data.role != "") {
      axios.post<SuccessResponse>("http://localhost:5000/admin/signin", data)
        .then((response) => {
          if (response.data.status) {
            dispatcher(signin({ admin: response.data.admin, token: response.data.token }));
            navigator("/");
          }
